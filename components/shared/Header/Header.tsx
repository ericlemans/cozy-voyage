...
const NAV_LINK = "block px-6 py-3.5 text-white/70 hover:text-white text-xs font-semibold uppercase transition-colors duration-150 border-b border-white/5";
...
export default function Header() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50">
      <header className="flex items-center justify-between px-6 md:px-12 py-4 bg-black/30 backdrop-blur-sm">
        ...
        <div className="hidden md:flex items-center gap-4">
          <Link
            href="/"
            className="text-white/70 hover:text-white text-xs font-semibold uppercase transition-colors duration-200"
            ...
            {locales.map((l, i) => (
              <React.Fragment key={l}>
                ...
                <button
                  ...
                  className={`px-2 cursor-pointer py-1 text-sm font-semibold tracking-widest uppercase transition-colors duration-200 ${
                    locale === l ? 'text-white' : 'text-white/40 hover:text-white/70'
                  }`}
                >
                  {l.toUpperCase()}
                </button>
        ...
        <div className="flex md:hidden items-center gap-3">
          <Link
            href={LODGIFY.all(locale)}
            target="_blank"
            className="inline-flex items-center bg-rose-600 hover:bg-rose-500 text-white text-xs font-semibold uppercase px-3 py-2 transition-colors duration-200"
            style={{ letterSpacing: '0.12em' }}
          >
            {t('cta')}
          </Link>
      ...
      {/* Mobile menu panel */}
      {menuOpen && (
        <nav className="md:hidden bg-black/95 backdrop-blur-sm border-t border-white/10">
          <Link href="/" onClick={close} className={NAV_LINK} style={{ letterSpacing: '0.12em' }}>
            {t('home')}
          </Link>
          <Link href="/about" onClick={close} className={NAV_LINK} style={{ letterSpacing: '0.12em' }}>
            {t('about')}
          </Link>

          {/* City Guide flat links */}
          <div className="border-b border-white/5">
            <p
              className="px-6 pt-3 pb-1 text-white/30 text-[10px] font-semibold uppercase"
            ...
            <Link
              href="/guide?city=berlin"
              onClick={close}
              className="block px-8 py-3 text-white/70 hover:text-white text-xs font-semibold uppercase transition-colors duration-150"
              style={{ letterSpacing: '0.12em' }}
            >
              Berlin
            </Link>
            <Link
              href="/guide?city=dresden"
              onClick={close}
              className="block px-8 pb-3 py-3 text-white/70 hover:text-white text-xs font-semibold uppercase transition-colors duration-150"
              style={{ letterSpacing: '0.12em' }}
            >
              Dresden
            </Link>
